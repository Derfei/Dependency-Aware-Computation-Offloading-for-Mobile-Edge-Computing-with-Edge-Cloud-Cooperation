...
def control_openface_greedy_1(algor_type=1, budget_type=1, double_type=1):
    ...
    time_cost_list[-1][0] = time.time()
    time_cost_list[-1][1] = time_cost_list[-1][0]
    tmptask = task(requestdeviceid, applicationid, offloadingpolicyid, 1, 1, np.array(input), [-1], [1, 2, 3, 4],
                   time_cost_list)
...
def control_vggboostvgg_cms1(algor_type=1, budget_type=1, double_type=1):
    ...
    time_cost_list[-1][0] = time.time()
    time_cost_list[-1][1] = time_cost_list[-1][0]
    tmptask = task(requestdeviceid, applicationid, offloadingpolicyid, 0, 0, np.array(x), [-1], [3],
                   time_cost_list)
...
def control_vgg_sa(algor_type=1, budget_type=1, double_type=1):
    import numpy as np
    import time
    from keras.preprocessing import image
    from keras.applications.imagenet_utils import preprocess_input
    from offloading import offloading_all_toedge

    requestdeviceid = local_deviceid
    applicationid = getRandomId()
    taskidlist = [i for i in range(5)]
    formertasklist = [[i - 1] for i in range(5)]
    nexttasklist = [[i + 1] for i in range(5)]
    nexttasklist[-1][0] = -1
    operationidlist = [i for i in range(5)]
