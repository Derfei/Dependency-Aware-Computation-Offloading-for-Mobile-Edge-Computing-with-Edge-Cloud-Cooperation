...
cloud_computing_capacity = 5  # 远程云服务器计算能力
...
eta = 2
e_for_energy = 1.5
wireless_bandwidth = 5   # 5MHz
transmission_rate_remote_cloud = 100   # 与远程云和edge的传输速率10Mbps
R_mnp = None
...
upload_rate_between_mobile_and_edge = None
# 计算Rmnp
def computer_R_mnp():
    global R_mnp
    if upload_rate_between_mobile_and_edge == None:
        R_mnp = wireless_bandwidth * math.log2(1 + Ptx*gain)
        return R_mnp
    else:
        R_mnp = float(upload_rate_between_mobile_and_edge)
        return R_mnp
...
def computer_TRf_mn(TFt_mn, TFf_mn, TFc_mn, pre, Tt=0, fog_exec_node_index=-1):
    if pre[0] == -1:
        return Tt
    ...
    for r in result:
        if r[0]:
            tmp[index] = TFc_mn[i]
        index += 1
    if fog_exec_node_index != -1:
        if sum(XIst[:, i]) != 0:
            if tmp.reshape(tmp.shape[0])[fog_exec_node_index] > max(TFf_mn[:, i]):
                return tmp.reshape(tmp.shape[0])[fog_exec_node_index]
            else:
                return max(TFf_mn[:, i])
        return tmp.reshape(tmp.shape[0])[fog_exec_node_index]
    return tmp.reshape(tmp.shape[0])

def computer_TRc_mn(TFt_mn, Tr_mn, TFc_mn, TFr_mn, pre, Tt_plus_Tr=0):
    if pre[0] == -1:
        return 0
    tmp = []
    for i in pre:
...
def computer_TFf_mn(Tf_mn, TRf_mn, fog_exec_node_index=-1):
    if fog_exec_node_index != -1:
        return (Tf_mn + TRf_mn)[fog_exec_node_index], TRf_mn[fog_exec_node_index]
...
def computer_Ec_mn(Tc_mn):
    return (alpha_c * ((cloud_computing_capacity/math.pow(10, 9))**e_for_energy) + beta_c) * Tc_mn

def computer_Tf_mn(W_mn,fe_p):
    return W_mn/fe_p

def computer_Ef_mn(tf_n):
    return (alpha_e * ((edge_computing_capacity/math.pow(10, 9))**e_for_energy) + beta_e) * tf_n
...
def get_min_workload_index(Ec_mn, x):
    tmp = np.where(Ec_mn * x>0)[0]
    min_index = 0
    min_value = 10000
    for index in tmp:
        if min_value > (Ec_mn * x)[index]:
            min_index = index
    return min_index
...
def update_finish_time(TFl_mn, TFf_mn, TFc_mn, Tl_mn, Tc_mn, Tf_mn, Tt_mn, Tr_mn, TFt_mn, TFr_mn, TRc_mn, TRf_mn, TRl_mn):
    TFl_mn[:] = 0
    TFf_mn[:, :] = 0
    TFc_mn[:] = 0
    TFt_mn[:] = 0
    ...
    for sub_task_index in range(sub_task_num):
        ...
        if Xc[sub_task_index] == 1:  # 在云上执行
            TRc_mn[sub_task_index] = computer_TRc_mn(TFt_mn[sub_task_index], Tr_mn, TFc_mn, TFr_mn[sub_task_index],
                                                     pre[sub_task_index], Tt_mn[sub_task_index] + Tr_mn[sub_task_index])
            TFc_mn[sub_task_index], lastTFc_mn = computer_TFc_mn(Tc_mn[sub_task_index], TRc_mn[sub_task_index])
        elif sum(XIst[:, sub_task_index]) == 1:  # 在edge上执行
            fog_exec_node_index = np.where(XIst[:, sub_task_index]==1)[0][0]
            TRf_mn[fog_exec_node_index, sub_task_index] = computer_TRf_mn(TFt_mn[sub_task_index], TFf_mn, TFc_mn, pre[sub_task_index], Tt_mn[sub_task_index], fog_exec_node_index)
            TFf_mn[fog_exec_node_index, sub_task_index], lastTFf_mn = computer_TFf_mn(Tf_mn[sub_task_index], TRf_mn[:, sub_task_index], fog_exec_node_index)
        else:
...
def run():
    ...
    for sub_task_index in range(sub_task_num):
        ...
        # if sub_task_pre[0] == -1:  # 首任务默认本地执行
            ...
            # TFl_mn[sub_task_index] = Tl_mn[sub_task_index]
            # continue
        ...
        TFr_mn[sub_task_index] = computer_TFr_mn(Tr_mn[sub_task_index], TFf_mn, pre[sub_task_index])
        TRl_mn[sub_task_index] = computer_TRl_mn(pre[sub_task_index], TFl_mn, TFf_mn, TFc_mn)
        TRf_mn[:, sub_task_index] = computer_TRf_mn(TFt_mn[sub_task_index], TFf_mn, TFc_mn, pre[sub_task_index], Tt_mn[sub_task_index])
        TRc_mn[sub_task_index] = computer_TRc_mn(TFt_mn[sub_task_index], Tr_mn, TFc_mn, TFr_mn[sub_task_index], pre[sub_task_index], Tt_mn[sub_task_index]+Tr_mn[sub_task_index])
        TFl_mn[sub_task_index], lastTFl_mn = computer_TFl_mn(Tl_mn[sub_task_index], TRl_mn[sub_task_index])
        TFf_mn[:, sub_task_index], lastTFf_mn = computer_TFf_mn(Tf_mn[sub_task_index], TRf_mn[:, sub_task_index])
        TFc_mn[sub_task_index], lastTFc_mn = computer_TFc_mn(Tc_mn[sub_task_index], TRc_mn[sub_task_index])

        if min(TFl_mn[sub_task_index], np.min(TFf_mn[:, sub_task_index]), TFc_mn[sub_task_index]) == TFl_mn[sub_task_index]:
            # 子任务在本地执行
    ...
    while C_mn > budget and iter > 0:
        iter -= 1
        min_edge_server_index = np.where(edge_server_workload==np.min(edge_server_workload))[0][0]
        min_workload_index = get_min_workload_index(Ec_mn, Xc)
        # 不满足约束12
        ...
        if np.sum(Xc) != 0:
            ...
            edge_server_workload[min_edge_server_index] += workload[min_workload_index]
        else:
            min_workload_index = get_min_workload_index(Ef_mn, np.sum(XIst, axis=0))
            XIst[:, min_workload_index] = 0
        C_mn = computer_C_mn(El_mn, Et_mn, EI_mn, Es_mn)
    ...
    while computer_Ue(Es_mn, Ef_mn, EI_mn) < 0 and iter > 0:
        ...
        if len(Es_mn_largethan_Ef_mn_array) == 0:
            break
        target_workload_index = Es_mn_largethan_Ef_mn_array[0]
        min_edge_server_index = np.where(edge_server_workload==np.min(edge_server_workload))[0][0]
        if np.sum(Xc) != 0 and np.max(Es_mn / Ef_mn) > 1 and edge_server_workload[min_edge_server_index] + workload[target_workload_index] < max_workload:
            Xc[target_workload_index] = 0
            XIst[min_edge_server_index, target_workload_index] = 1

    # print("XIst", XIst)
    # print("Xc", Xc)

    last_task = len(pre) -1
    update_finish_time(TFl_mn, TFf_mn, TFc_mn, Tl_mn, Tc_mn, Tf_mn, Tt_mn, Tr_mn, TFt_mn, TFr_mn, TRc_mn, TRf_mn, TRl_mn)
    on_edge = np.multiply(XIst, TFf_mn)
    last_time_spend = (1-sum(XIst[:, last_task]))*(1-Xc[last_task])*TFl_mn[last_task] +\
                      sum(XIst[:,last_task])*sum(on_edge[:, last_task]) + Xc[last_task]*TFc_mn[last_task]
    # print("The time cost: {0} The Cost is: {1}".format(last_time_spend, np.sum(C_mn)))
    return Xc, XIst, last_time_spend, np.sum(C_mn)

def setTaskGraphGCC(graph_type):
    '''
    设置不同的任务图类型
    :param graph_type: 1： VGG 2： VGGboostVGG 3: RESNET NET 50 4: OpenFace
    :return:
    '''
    import os
    if graph_type == 1:
        ...
        data_size = (data_size * 4) / (1024 * 1024)
...
def setGccBudget(budg):
    global budget
    budget = budg

def setGccUpload_rate_between_mobile_and_edge(rate):
    global  upload_rate_between_mobile_and_edge
    upload_rate_between_mobile_and_edge = rate

def setDatasize(img_count):
    global  data_size
    data_size = img_count * data_size

if __name__ == '__main__':
    # workload for resnet50         workload = '0.024611516	0.008945174	0.007231941	0.00775403	0.007250237	0.021307387	0.010021753	0.009001012	0.008428769	0.008337641	0.025387111	0.01052021	0.009481444	0.008310304	0.014070501	0.007180567	0.006811976	0.006734285	0.006838336	0.014363494	0.007285872	0.00706141	0.005310335	0.007902012	0.004875331	0.00447156	0.004709363	0.006109929	0.004855528	0.00402976	0.004211988	0.006221547	0.002709641'.split()
    # workload for openface 33      workload = '0.066439695	0.051343827	0.038689017	0.336932201	0.082959342	0.337392921	0.083378024	0.343366885	0.076267123	0.074381781	0.202175264	0.055702724	0.194068999	0.051175013	0.195705881	0.054172029	0.194763641	0.04793674	0.044750166	0.125573778	0.036624289	0.124879093	0.036311903	0.125546317	0.037398782	0.125342145	0.037574887	0.126161351	0.037093277	0.130339127	0.036166987	0.036475015	0.089209728	0.029032798	0.087064009	0.027950749	0.088542018	0.016066232'.split()
    # pre = [[-1], [0], [0], [1, 2], [3], [3, 4], [5], [5, 6], [7], [7], [8, 9], [10], [10, 11],
    #        [12], [12, 13], [14], [14, 15], [16], [16], [17, 18], [19], [19, 20], [21], [21, 22],
    #        [23], [23, 24], [25], [25, 26], [27], [27, 28], [29], [29], [30, 31], [32], [32, 33],
    #        [34], [34, 35], [36]]
    ...
    setTaskGraphGCC(3)
    setGccBudget(100)
    setGccUpload_rate_between_mobile_and_edge(1.2)
    Xc, Xist, _, _  = run()
    print("The count of task run on the cloud is: {0} The count of task run on the edge is: {1} Budget is {2}".format(
        sum(Xc), np.sum(Xist), budget
    ))